...
- `chat`、`models`、`embeddings`、`rerank`、`healthz`、`admin status` 六类主线入口
- `Base URL`
- `Model`
- `Inference Backend`
- chat 的 `Stream / Non-stream`
- 右侧固定高度、内部滚动的响应窗口
- `Finish`、`TTFB`、`Duration`
- 截断后自动续写

agent preset、agent tools 和实验模式仍保留，但已降级到“Compatibility Extensions”折叠区，只对 chat 请求生效。
