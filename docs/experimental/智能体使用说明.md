...
本文档描述当前仓库里已经实现的 Agent 行为，重点说明 `code_analysis` 与 `web_research` 两种模式的真实能力、调用链、调试方式和限制。当前对外主能力是只读、证据优先的 `code_analysis` agent；`web_research` 是受控增强能力，用于外部资料交叉验证。
...
- 默认模式、主推模式都是 `code_analysis`
- 优先回答仓库、代码、配置、接口、流式链路、服务状态相关问题
- 通过工具先取证，再总结
- 不支持写文件、patch 或 shell
- `web_research` 模式下支持受控外部网页搜索，但不会暴露任意 shell/curl 联网能力
- `generic` 仅保留内部实验用途，不作为对外公开模式

一句话概括：

> 它是一个“仓库内证据优先、结构化输出优先”的只读分析 agent；其中 `code_analysis` 是主模式，`web_research` 是交叉验证增强模式。

## 2. 模式边界

- `code_analysis`
  主推模式。优先仓库、本地文档、配置、运行时状态等本地证据，适合代码理解、接口定位、调用链和排障。

- `web_research`
  补充模式。仍然先以仓库和本地证据为主，只有在需要外部资料或最新公开信息交叉验证时，才补充受控 `search_web / fetch_url`。

- `generic`
  保留内部实验用途。HTTP 对外文档、demo 和调试接口都不把它作为主宣传能力。
...
`web_research` 只在确实需要外部资料交叉验证时再打开，例如：
...
```json
{
  ...
  "messages": [
    {
      "role": "user",
      "content": "结合仓库和 http://example.com/ 页面，说明 web_research 的 references 是否会保留外部 URL。"
    }
  ]
}
```
...
- `agent_mode`
  ...
  未传时默认仍是 `code_analysis`。`generic` 保留内部实验用途，不作为公开接口模式。
...
- 模型不负责底层工具策略，只负责最后总结
- `code_analysis` 是主链路，优先 repo/local 证据
- `web_research` 先拆分子查询，再按 repo/docs/web 选受控工具做交叉验证
- 当前冻结新增能力，除 bug、trace、references、返回格式、评测、文档问题外，不继续新增新工具、新模式、新 planner 分支

## 8. 主执行链路

### 8.1 chat/completions 主链路
...
- `search_web -> fetch_url -> synthesis`
- `structured output`
- `debug trace`
...
- SSE、`Transfer-Encoding: chunked`、`Content-Length`、`411 Length Required` 等“仓库实现 + 外部资料交叉验证”样本
...
- 它是增强能力，不替代 `code_analysis` 作为主推模式

因此当前最准确的说法是：

> 它是一个只读、证据优先、统一结构化输出的研究 agent；其中 `code_analysis` 是主推的仓库分析模式，`web_research` 是用于外部资料交叉验证的增强模式。
