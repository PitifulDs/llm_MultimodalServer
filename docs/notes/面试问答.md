...
3. HTTP 到推理的完整链路是什么？
答: NetworkHttpServer 解析 HTTP → HttpGateway 做校验/治理并构建上下文 →（chat：SessionExecutor 串行 → EngineExecutor 按 capability+model+backend 排队）→ LlamaEngine/StackFlowEngine 推理 → 返回 JSON 或 SSE。
...
80. `/v1/models` 里的 `declared_backends / backends / gateway_backends` 怎么讲清楚？
答: 先讲“内部命名 vs 对外命名”，再讲“配置能力 vs 网关切换能力”：
- `declared_backends`：配置里声明的后端能力（内部命名 `local/stackflow`）
- `backends`：网关对外展示的可用后端（对外命名 `local/rpc`，其中 `rpc` 在内部归一为 `stackflow`）
- `gateway_backends`：网关支持的请求级后端切换能力（对外命名 `local/rpc`），不等价于配置里一定为每个模型写了两套 backend

81. EngineExecutor 的排队维度是什么？为什么这样设计？
答: route key 是 `capability||model||resolved_backend`。这样同一模型的不同能力（chat/embeddings/rerank）不会互相影响，同一能力在不同后端（local/stackflow）也能隔离，同时又能对冲突维度做串行保护。

82. embeddings / rerank 为什么现在是同步执行？是否有隐患？
答: 当前实现选择“handler 同步执行 + 治理保护”，链路更短、实现更直接；隐患是它不走 EngineExecutor 的排队背压链路，遇到慢后端时更依赖全局/模型并发限制与 HTTP 超时兜底。

83. 你们的全局/模型/会话并发限制怎么做的？
答: 通过 `AcquireRequestLease()` 返回一个 RAII 租约 token；token 存活期间占用并发额度，析构时自动归还。并发限制分三层：`rate_limit_global` / `rate_limit_model` / `rate_limit_session`。

84. 客户端断连如何影响同步接口（embeddings/rerank）？
答: 同步接口会启动一个 watchdog 轮询 `res.IsAlive()`；一旦断连就把 `cancelled` 置 true，service 在后端执行失败时按统一口径映射为 `request_cancelled`/timeout/error，避免继续无效计算。

85. `WORKER_THREADS` 从哪里来？面试官追问“能不能动态改”怎么答？
答: `serving_http_server` 启动时会从 `config.json`（`worker_threads`）写入环境变量 `WORKER_THREADS`，`HttpGateway` 构造时读取并创建线程池。当前实现是“启动时确定”，运行中不支持动态扩缩容（需要额外的线程池重建与迁移策略）。
